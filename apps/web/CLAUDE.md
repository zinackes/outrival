...
- Icônes : `@phosphor-icons/react/ssr` uniquement (le sous-chemin `/ssr`, jamais l'entrée racine : elle est context-aware donc client-only, et la moitié des pages qui portent des icônes sont des server components). Le type d'un composant icône s'importe en `import type { Icon as PhosphorIcon } from "@phosphor-icons/react"` (`Icon` seul entrerait en collision avec le `const Icon = …` des rendus dynamiques). Noms canoniques suffixés `Icon` (`CheckIcon`, `CaretRightIcon`) — les alias non suffixés sont dépréciés. Glyphes remplis, pas des traits : l'épaisseur se règle avec `weight` (`bold` sur les micro-tailles), jamais avec `strokeWidth`