...
- **Grille de taille des icônes : 16 / 20 / 24, jamais en dessous de 16.** `size-4` / `size-5` / `size-6` côté classes. Pas de prop `weight` (elle était propre à Phosphor) et jamais de `strokeWidth` au cas par cas : une seule graisse pour tout le produit.
