...
- Typo : **Geist Sans pour display/titres ET corps/UI** (une seule grotesque neutre — la hiérarchie vient du poids/taille/tracking, pas d'une police de titre typée) + Geist Mono (tabular-nums + slashed-zero) ; landing = Zodiak serif (registre brand) — `globals.css` + `layout.tsx`
- **Les CHIFFRES sont en sans, pas en mono** (DESIGN.md §3 « Numbers-Are-Sans Rule »). Un prix, un compteur, un score, une date, un rang, un delta, un KPI : `tabular-nums` sur du Geist Sans — c'est l'utilitaire qui aligne la colonne, la police mono n'ajoutait qu'une 2e texture sur la même ligne que la prose. Geist Mono reste pour ce qui se lit glyphe par glyphe et où un caractère lu de travers change le sens : `<kbd>`, backup codes / secret TOTP, IDs et noms de tâche, URLs / hostnames / slugs, noms de header, marqueurs `+`/`-` d'un diff. La console `/admin` est hors périmètre (terminal ops, ses `style={mono}` restent).
