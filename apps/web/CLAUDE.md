...
- Icônes : `@/components/icons` uniquement — plus aucun package d'icônes. Le module inline le tier **gratuit d'Iconsax** en style `linear` (grille 24, `stroke-width` 1.5, `currentColor`), récupéré via leur MCP officiel ; le header du fichier documente la provenance, la licence et comment en ajouter une. Noms suffixés `Icon` (`CheckIcon`, `CaretRightIcon`) ; le type d'un composant icône s'importe en `import type { Icon as PhosphorIcon } from "@/components/icons"` (`Icon` seul entrerait en collision avec le `const Icon = …` des rendus dynamiques ; l'alias garde son nom historique pour ne pas toucher les tables de lookup). Le loader est `SpinnerIcon` et n'anime pas tout seul — c'est l'appelant qui pose `animate-spin`.
- **Grille de taille des icônes : 16 / 20 / 24, jamais en dessous de 16.** Un trait de 1.5 sur 24 rendu à 13px fait 0.81px : sous-pixel, donc flou. 16px donne 1.0px pile. `size-4` / `size-5` / `size-6` côté classes. Pas de prop `weight` (elle était propre à Phosphor) et jamais de `strokeWidth` au cas par cas : une seule graisse pour tout le produit.