...
function SidebarProvider({
  ...
  // Dismiss the mobile off-canvas sheet whenever navigation lands on a new route:
  // tapping a nav link both redirects and closes the overlay instead of leaving it
  // open over the page. Keyed on pathname only, so same-page ?product= swaps / refreshes
  // don't collapse it. No-op on desktop (the sheet isn't rendered).
...
function Sidebar({
  ...
  // lg, not md, and paired with MOBILE_BREAKPOINT in @/hooks/use-mobile — the two
  // must name the same width or a tablet gets both the off-canvas Sheet and the
  // permanent rail. See the comment there for why 768 was the wrong line (`ux:82`).
  return (
    <div
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        // min-w-0: let the inset shrink below its content's intrinsic width so wide
        // children (e.g. the Compare table) scroll inside their own overflow-x-auto
        // box instead of blowing the whole page out horizontally past the sidebar.
