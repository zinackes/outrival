...
// Trigger.dev wrapper in ../jobs/ai-visibility-teaser.job.ts (deleted at the
...
export async function runAiVisibilityTeaser(payload: z.input<typeof InputSchema>) {
...
// to "unavailable". onConflictDoNothing so we never clobber a "ready" a prior run
// wrote. Trigger calls it via the wrapper's onFailure; the pg-boss handler calls it
// on the terminal attempt.
export async function onAiVisibilityTeaserFailure({ payload }: { payload: unknown }) {
