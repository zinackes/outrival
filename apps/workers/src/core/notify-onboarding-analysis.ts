...
import { refreshCompetitorSummary } from "@outrival/queue";
import { wait } from "../lib/job-wait";
import { z } from "zod";
...
// Trigger.dev wrapper in ../jobs/notify-onboarding-analysis.job.ts (deleted at the
// cutover). Only the header, the signature and the fan-out call change.
export async function runNotifyOnboardingAnalysis(payload: z.input<typeof InputSchema>) {
      ...
      if (tick >= 1) {
        ...
        if (missing.length > 0) {
          ...
          for (const { competitorId } of captured) {
            ...
            await refreshCompetitorSummary.enqueue({ competitorId });
          }
