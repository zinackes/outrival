...
// Trigger.dev wrapper in ../jobs/schedule-platform-detection.job.ts (deleted at
// the cutover). Only the header, the signature and the fan-out call change.
export async function runSchedulePlatformDetection() {
    ...
    await detectPlatform.enqueueMany(due.map((c) => ({ data: { competitorId: c.id } })));
