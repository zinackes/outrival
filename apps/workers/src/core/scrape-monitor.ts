...
const SIZE_VARIABLE_SOURCES = new Set(["blog", "changelog", "news", "sitemap", "subdomains", "youtube", "hackernews", "wellknown", "docs"]);
...
const SYNTHETIC_DOC_SOURCES = new Set(["sitemap", "news", "github_repo", "subdomains", "youtube", "hackernews", "wellknown", "docs"]);
...
async function maybeRedetectPlatformOnDrift(
  ...
  try {
    ...
    if (!atsDrift) return;
    await detectPlatform.enqueue({ competitorId: competitor.id });
...
export async function runScrapeMonitor(payload: z.input<typeof InputSchema>) {
      ...
      if (beforeHtml !== null) {
        ...
        if (otherAdded.length > 0 || removed.length > 0) {
          ...
          if (expansionChange) {
            ...
            changedAt = new Date();
            await classifyChange.enqueue({ changeId: expansionChange.id });
          }
        }
    ...
    if (extractionAllowed && monitor.sourceType === "homepage" && competitor.type === "self") {
      await extractSelfProfile.enqueue({
        competitorId: competitor.id,
...
// Runs once after all retries are exhausted — the anti-flood guard. Persist the
// failure so the UI can show a "failed" state instead of spinning until the client
// poll times out, and clear the in-progress marker. Trigger calls it via the
// wrapper's onFailure; the pg-boss handler calls it on the terminal attempt.
export async function onScrapeMonitorFailure({
  payload,
  error,
}: {
  payload: unknown;
