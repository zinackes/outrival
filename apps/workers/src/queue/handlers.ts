...
//   browser — the jobs that launch Chromium or render a PDF
//             (scrape-monitor, detect-platform, generate-battle-card).
//             Deployed from Dockerfile.queue-browser (browsers baked in, big RAM).
...
export async function registerHandlers(role: WorkerRole): Promise<string[]> {
  ...
  if (role === "browser") {
    // Phase 3: scrapeMonitor, detectPlatform, generateBattleCard.
  }
