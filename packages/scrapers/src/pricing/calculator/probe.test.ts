...
describe("probeCalculator (live browser)", () => {
  it.skipIf(!hasBrowser)(
    ...
    async () => {
      ...
      expect(out.shots.map((s) => s.qty).sort((a, b) => a - b)).toEqual(QUANTITIES);
      for (const shot of out.shots) expect(shot.png.length).toBeGreaterThan(0);
  ...
  it.skipIf(!hasBrowser)(
    "reads the total off the page's own pricing endpoint when there is one",
    async () => {
