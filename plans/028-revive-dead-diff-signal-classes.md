...
> `git diff --stat d153e36..HEAD -- packages/scrapers/src/diff packages/scrapers/src/scoring packages/ai/src/filters/significance.ts apps/workers/src/jobs/scrape-monitor.job.ts`
> If any in-scope file changed since this plan was re-anchored, compare the
> "Current state" excerpts against the live code before proceeding; on a
> mismatch, treat it as a STOP condition.
>
> **Base your branch on `origin/main` (`d153e36`).**
...
- **Re-anchored at**: commit `d153e36` (origin/main), 2026-07-09 — plan 025 (PR #155)
  shifted `scrape-monitor.job.ts` by ~50 lines. Only that file drifted; `diff/`,
  `scoring/` and `significance.ts` are byte-identical to `6639163`. Line references
  below are updated. Branch off `origin/main` (`d153e36`).
...
- `apps/workers/src/jobs/scrape-monitor.job.ts` (structured homepage branch)
  — line numbers below are at `d153e36`, re-verified:
  - Recency count query (**lines 1066–1073**): `.from(changes)` +
    `.innerJoin(monitors, eq(changes.monitorId, monitors.id))` (line 1069), filtered
    on the competitor — **competitor-global**, all source types. Result feeds
    `previousChangesInLast7Days` (line 1073) → `scoreRelevance(...)` (line 1077).
  - Below-threshold changes are silenced with only a log (just below) — context for
    why these fixes matter; do not change that mechanism.
  - Significance gate call (**line 1210**): `evaluateSignificance({ added, removed })`
    → `worth: false` skips `classify-change` entirely (change row still recorded).
    Imported at line 30 from `@outrival/ai/significance`.
...
In `apps/workers/src/jobs/scrape-monitor.job.ts` (**lines 1066–1073** at `d153e36`), change
the count query to this monitor only — drop the `.innerJoin(monitors, ...)` at line 1069:
...
`sed -n '1060,1080p' apps/workers/src/jobs/scrape-monitor.job.ts` shows no
...
2. Call site `apps/workers/src/jobs/scrape-monitor.job.ts:1210` (at `d153e36`): pass
   `{ sourceType: monitor.sourceType }` as the second argument. NOTE: `evaluateSignificance`
   has a second caller, `apps/workers/src/jobs/backfill-history.job.ts` — it must keep
   compiling untouched, which the optional param guarantees. Do not edit it.
...
- [ ] `grep -n "innerJoin" apps/workers/src/jobs/scrape-monitor.job.ts` → the recency query (lines ~1066–1073) no longer joins monitors
...
- `stableSectionTransitions` / `filterUnstableSections` have changed since `6639163`
  (someone may have fixed DIF-1 differently — reconcile, don't stack). These files are
  confirmed byte-identical at `d153e36`, so any mismatch means real drift.
