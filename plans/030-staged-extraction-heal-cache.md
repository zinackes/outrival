...
> `git diff --stat 0d35d0f..HEAD -- apps/workers/src/lib/staged-extract.ts packages/scrapers/src/parsers/cached-extractor.ts packages/ai/src/tasks/generate-extractor.ts packages/db/src/schema/parser-extractors.ts`
...
- **Re-anchored at**: commit `0d35d0f` (origin/main), 2026-07-10 — PR #150
  (`fix/extractor-revalidation`, audit R8) restructured `staged-extract.ts`
  (201→227 lines): a cached spec now EXPIRES via `shouldTrustCachedExtractor`
  (`apps/workers/src/lib/extractor-trust.ts`), so stage-2 replay is nested inside an
  `if (trusted)` block and two env vars were added (`EXTRACTOR_REVALIDATE_INTERVAL_DAYS`,
  `EXTRACTOR_MAX_CONSECUTIVE_FAILURES`). **This did NOT fix SCR-20**: the shape mismatch
  (`replayExtractor` returns a bare rows array, `stageOk` validates it against the
  object schema) is still live at BOTH stage 2 and stage 3, so cache/heal still never
  persists. The R8 trust layer is COMPLEMENTARY and benign for this fix —
  `shouldTrustCachedExtractor` returns `false` for a null `lastValidatedAt` (verified),
  so the step-3 stub row is correctly distrusted → re-heal (cooldown-gated).
  `replayExtractor`'s bare-array contract and the two nullable columns
  (`lastValidatedAt`/`lastHealAttemptAt`) are unchanged. **Match the CODE excerpts
  below, not line numbers** (R8 shifted them all).
...
All excerpts verified at `origin/main` = `6639163`; re-verified at `0d35d0f` (R8
restructured the file — code shapes below are current, line numbers are gone):

- `apps/workers/src/lib/staged-extract.ts` (227 lines, post-#150/R8) — the orchestrator:
  - `validateSchema` / `stageOk` run `input.schema.safeParse(raw)` on the RAW replay
    value; `input.schema` is the source's OBJECT schema (`z.object({ plans })` /
    `z.object({ jobs })`) so a bare rows array never parses — the SCR-20 core.
  - `input.kind` (`"pricing" | "jobs"`, `ExtractorKind` from `@outrival/ai`) is a field
    on `StagedExtractInput` and in scope at both stages below.
  - **Stage 2** cache replay is now nested inside an `if (trusted)` block (R8's
    `shouldTrustCachedExtractor`). The exact line to wrap:
    `const replayed = stageOk(replayExtractor(input.html, cachedSpec.data));`
    (a schema-valid replay resets `consecutiveFailures: 0`; a failed one increments it).
  - **Stage 3** heal lives in `if (!inCooldown) { try { const spec = await loggedAi("generate_extractor", …); if (spec) { const version = (cached?.version ?? 0) + 1; const persisted = { ...spec, version }; const healed = stageOk(replayExtractor(input.html, persisted)); if (healed) { await upsertExtractor(…); return finish(healed, "heal", version); } } /* stamp block here */ } catch (err) { … } }`.
    The exact line to wrap: `const healed = stageOk(replayExtractor(input.html, persisted));`.
  - The "generated but didn't validate" stamp is still guarded `if (cached) { …set({ lastHealAttemptAt: new Date() })… }` (the `/* stamp block here */` position above, inside the `try`, after the `if (spec) {…}`) — with an empty table `cached` is undefined → **cooldown never armed**.
  - `upsertExtractor` (bottom of file): the ONLY writer of `parser_extractors`; upserts
    on `(domain, sourceType)`, stamps `lastValidatedAt: now`, resets `consecutiveFailures: 0`.
...
In `apps/workers/src/lib/staged-extract.ts` (match the code, not line numbers — R8
shifted them; there is exactly one `stageOk(replayExtractor(...))` at each stage):

- Add the import at the top with the other local-lib imports:
  `import { normalizeReplayOutput } from "./replay-normalize";`
- **Stage 2** — the `const replayed = stageOk(replayExtractor(input.html, cachedSpec.data));`
  line (inside the `if (trusted)` block) becomes:
  `const replayed = stageOk(normalizeReplayOutput(input.kind, replayExtractor(input.html, cachedSpec.data)));`
- **Stage 3** — the `const healed = stageOk(replayExtractor(input.html, persisted));`
  line becomes:
...
Replace the current "generated but didn't validate" stamp block — which reads
exactly:
...
(it sits right after the `if (spec) { … if (healed) return … }` block, inside the
`try`, before the `catch`; `spec`, `cached`, `domain`, `db`, `parserExtractors`, `eq`
are all already in scope) — so a generated-but-not-validating spec is persisted as a
**stub row** (this both arms the cooldown and lets a later run track the broken spec
via `consecutiveFailures`; R8's `shouldTrustCachedExtractor` returns false for the
stub's null `lastValidatedAt`, so it is never replayed as a trusted cache):
